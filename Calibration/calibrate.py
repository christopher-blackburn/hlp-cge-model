...
def calibrate():
    ...
    # Compute the mean index for each country
    mean_chn = [np.mean(chn_string[0:90]) for i in range(0,90)]
    mean_usa = [np.mean(usa_string[0:90]) for i in range(0,90)]
    mean_row = [np.mean(usa_string[0:90]) for i in range(0,90)]


    # Compute the means for each month of the first quarter
    # January
    mean_chn_mo1 = [np.mean(chn_string[0:30]) for i in range(0,30)]
    mean_usa_mo1 = [np.mean(usa_string[0:30]) for i in range(0,30)]
    mean_row_mo1 = [np.mean(usa_string[0:30]) for i in range(0,30)]

    # February
    mean_chn_mo2 = [np.mean(chn_string[30:60]) for i in range(30,60)]
    mean_usa_mo2 = [np.mean(usa_string[30:60]) for i in range(30,60)]
    mean_row_mo2 = [np.mean(usa_string[30:60]) for i in range(30,60)]

    # March
    mean_chn_mo3 = [np.mean(chn_string[60:90]) for i in range(60,91)]
    mean_usa_mo3 = [np.mean(usa_string[60:90]) for i in range(60,91)]
    mean_row_mo3 = [np.mean(usa_string[60:90]) for i in range(60,91)]


    # Return the impacts
    mean_chn_mo1 = mean_chn_mo1[0]
    mean_chn_mo2 = mean_chn_mo2[0]
    mean_chn_mo3 = mean_chn_mo3[0]

    mean_usa_mo1 = mean_usa_mo1[0]
    mean_usa_mo2 = mean_usa_mo2[0]
    mean_usa_mo3 = mean_usa_mo3[0]

    mean_row_mo1 = mean_row_mo1[0]
    mean_row_mo2 = mean_row_mo2[0]
    mean_row_mo3 = mean_row_mo3[0]

    # Get the montly impact vectors
    chn_shock_mean1 = np.array(oes_tel_cap['non_tel_cap']*mean_chn_mo1).reshape((56,1))
    chn_shock_mean2 = np.array(oes_tel_cap['non_tel_cap']*mean_chn_mo2).reshape((56,1))
    chn_shock_mean3 = np.array(oes_tel_cap['non_tel_cap']*mean_chn_mo3).reshape((56,1))

    row_shock_mean1 = np.array(oes_tel_cap['non_tel_cap']*mean_row_mo1).reshape((56,1)) 
    row_shock_mean2 = np.array(oes_tel_cap['non_tel_cap']*mean_row_mo2).reshape((56,1)) 
    row_shock_mean3 = np.array(oes_tel_cap['non_tel_cap']*mean_row_mo3).reshape((56,1)) 


    usa_shock_mean1 = np.array(oes_tel_cap['non_tel_cap']*mean_usa_mo1).reshape((56,1)) 
    usa_shock_mean2 = np.array(oes_tel_cap['non_tel_cap']*mean_usa_mo2).reshape((56,1)) 
    usa_shock_mean3 = np.array(oes_tel_cap['non_tel_cap']*mean_usa_mo3).reshape((56,1)) 


    Xi_j_mean1 = np.concatenate((chn_shock_mean1,row_shock_mean1,usa_shock_mean1),axis=0)
    Xi_j_mean2 = np.concatenate((chn_shock_mean2,row_shock_mean2,usa_shock_mean2),axis=0)
    Xi_j_mean3 = np.concatenate((chn_shock_mean3,row_shock_mean3,usa_shock_mean3),axis=0)
    ...
    print('--------------------------------------------------------')
    print('---------------Model Successfully Calibrated------------')
    print('--------------------------------------------------------')

    return [Pi_f, Pi_x, chn_alpha_j, chn_eta_j, row_alpha_j, row_eta_j, usa_alpha_j, usa_eta_j, Upsilon, Psi_x, Psi_f,Xi_j_mean,Xi_j_max,usa_domar,Xi_j_mean1,Xi_j_mean2,Xi_j_mean3]
